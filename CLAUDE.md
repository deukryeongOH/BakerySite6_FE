...
- **아직 안 고쳐진 것 — order 도메인:** 주문 데이터(판매자/가격/상품명)가 백엔드에 스텁 상수로 고정돼 있고(`OrderService.STUB_*`), 취소 시 재고 복구·드롭마감 체크·중복요청 방지 전부 미구현, `GET /api/v1/seller/orders`(판매자 주문 목록)도 없음 — order를 붙일 때는 `docs/order-api.md`의 ⚠️ 문단을 먼저 확인할 것.
