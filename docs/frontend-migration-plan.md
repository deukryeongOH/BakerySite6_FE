...
6. **M5 드롭/장바구니/주문 — 코드 완료(2026-07-28), 라이브 e2e는 백엔드 버그로 보류.** 구현: `lib/api/drop.ts`/`cart.ts`/`order.ts`, 홈(오늘의 드롭), 드롭 상세(찜 + 대기열 진입→순번 폴링→입장확정), 주문/결제(`lock-start`→`POST /cart`→픽업일→`POST /orders`), 주문 완료/목록/상세(취소 포함), 찜(localStorage, memberId별). `npm run lint`/`npm run build` 통과.
   - ⚠️ **`GET /drops/{id}/info`, `GET /drops/today/drop`는 문서와 달리 실제로는 인증 필요** — 토큰 없이 호출하면 403(문서엔 "인증 불필요한 공개 API"로 돼 있음). 이 때문에 애초 계획했던 "정적 정보는 서버 컴포넌트로" 방식(위 서버/클라이언트 컴포넌트 경계 절 참고)은 포기하고 드롭 상세를 전부 클라이언트 컴포넌트로 되돌렸다(`app/(shop)/drops/[dropId]/page.tsx`). `lib/api/client.ts`의 `apiRequest`에 `unwrapped` 옵션을 추가해 `{success,data}` 래퍼 없는 응답(이 API만 해당)도 재발급 로직을 그대로 재사용해 처리한다.
   - ⚠️ **백엔드 버그(미수정, 프론트 작업 범위 밖)**: `GET /drops/{id}/info`와 `GET /drops/mine`이 500(`LazyInitializationException`, `pickUpAvailableDate` 컬렉션을 세션 밖에서 직렬화)을 던진다. `DropService.getDropProductInfo`(`DropService.java:133`)에만 `@Transactional`이 빠져 있는 게 원인으로 보임 — 백엔드 레포에서 별도로 고쳐야 함. 2026-07-28 로컬 검증 시 이 버그로 브라우저 e2e를 끝까지 못 돌렸고, 코드는 문서 스펙대로 작성됐으니 백엔드 수정 후 재검증 필요.
   ...
   - 검증 남은 부분: 위 백엔드 버그 수정 후 대기열 진입→순번 폴링→입장확정→재고선점→장바구니→픽업일→결제→주문내역→취소까지 브라우저로 전체 e2e 재확인 필요.
