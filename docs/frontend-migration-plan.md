...
6. **M5 드롭/장바구니/주문** — 대기열→선점→장바구니→결제 전체 플로우. drop/cart 쪽은 2026-07-28 백엔드 갱신으로 `GET /cart` 응답의 `drop`/`seller`/`estimatedAmount`/`pickupDates`가 이제 실제 값으로 채워지고, `GET /drops/{id}/info`(`ApiResponse` 래퍼 없이 응답이 최상위로 옴, 주의)로 드롭 상세도 조회 가능해짐 — 화면에서 로컬 상태로 임시 보완할 필요 없이 API 응답을 그대로 쓰면 됨. 다만 **order 쪽은 여전히 TODO 많음**: 주문 데이터가 스텁 상수(seller/가격/상품명), `dropCloseAt` 항상 null이므로 `cancelable` 불리언만 신뢰, 취소 시 재고 미복구. 검증: 대기열 진입부터 주문 취소까지 전체 e2e, order 취소는 재고 미복구 특성 감안해 테스트 데이터 넉넉히.
